  ...
  return (
    <main className="flex flex-col h-full" role="main">
      ...
      <div className="p-4" aria-label="Chat input">
        <ChatView chatPlaceholder="Type message" />
      </div>
    </main>
  )
}
...
import { ChatSkeleton } from "@/components/chat-skeleton"
import { IntegratedChatInput } from "@/components/integrated-chat-input"
// Interfaces
import { Message } from "@/constants/interfaces"
// Hooks
import { useChat, useSendMessage, useCreateChat } from "@/hooks/use-chats"

export default function ChatView({ id, chatPlaceholder = "Type message" }: { id?: string, chatPlaceholder?: string }) {
    const router = useRouter();
    const { data: chat, isLoading, isFetching } = useChat(id || "")
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="p-4">
                {/*make it pure components */}
                <IntegratedChatInput
                    placeholder={chatPlaceholder}
                    onSend={(message, attachments) => handleSendMessage(message, attachments)}
                />
            </div>
        </div>
    )
}
IntegratedChatInput(client component)
"use client"
